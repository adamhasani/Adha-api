...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function openApiModal(item) {
    ...
    if (DOM.endpointText) DOM.endpointText.value = url;
  ...
  function initModalEvents() {
    if (DOM.copyEndpointBtn && DOM.endpointText) {
      DOM.copyEndpointBtn.addEventListener("click", async () => {
        try {
          await navigator.clipboard.writeText((settings && settings.copyBaseUrl ? settings.copyBaseUrl : `${location.protocol}//${location.host}`) + DOM.endpointText.value.trim());
        } catch {
          // ignore
        }
      });
...
// ================================
// MANUAL EXECUTE (TRY BUTTON IN MODAL)
// ================================
document.addEventListener("DOMContentLoaded", () => {
  const tryExecuteBtn = document.getElementById("tryExecuteBtn");
  if (tryExecuteBtn) {
    tryExecuteBtn.addEventListener("click", () => {
